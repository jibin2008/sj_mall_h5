		...
		<div class="uni-shelve-content pro_cont" style="padding-top: 20rpx;padding-bottom: 100rpx;">
			...
			<div style="margin-left: 40rpx;margin-top: 20rpx;">
				<div v-for="(item, index) in shelvePos" :key="item.value" style="display: inline-block;text-align: left;margin-top: 10px;">
					<div>
						<checkbox :value="item.shelvePosition" :checked="item.checked" :disabled="item.num >= item.limit" @click.stop="shiftPositionCheckbox(index)"/>
						{{item.positionName}}
						<text style="font-size: 12px;color: #9c9c9c;">(还可以添加<text style="color: #F00;">{{diffNaturalNumber(item.limit,item.num)}}</text>款)</text>
					</div>
				</div>
			</div>
	<!-- 		<div style="margin: 10rpx auto 0 auto;">
				<text style="margin-top: 5px;margin-right: 5px;font-size: 14px;color: #444;">轮播图片：</text>
				<div class="pro_file_wrap" @click="uploadCarouselImage" style="display: inline-block;"><input type="file" style="opacity:0;" :value="carouselImageUrl"/></div>
			</div> -->
			<div style="margin: 15rpx auto;">
				<button type="warn" @click.stop="shelveProd()" class="load_btn04 tc fl yahei f16">上架</button>
	...
	export default {
		...
		methods: {
			...
			shiftPositionCheckbox(index) {
				...
				console.log('>> checkStatus: '+this.shelvePos[index].checked);
				if(this.shelvePos[index].num < this.shelvePos[index].limit){//有空位
					...
					uni.showToast({
						icon: 'none',
						title: this.shelvePos[index].positionName+'位置产品数量超过限制！'
					});
				}
			},
			/**
			...
			shelveProd() {
				...
				// 	return;
				// }
				console.log('shelveProd() - this.shelveData: '+this.shelveData);
				this.$emit('commitShelve', {...this.shelveData, productId:this.prodId}, ()=>{
					this.popup.close();
					this.prodLable.forEach((item,index)=>{
						item.checked = false;
					})
			...
			uploadCarouselImage : function(){
				...
				uni.chooseImage({
					...
					sourceType: ['album'], //album从相册选图，camera使用相机，默认二者都有
					success: function (res) {//成功则返回图片的本地文件路径列表tempFilePaths
						const tempFilePaths = res.tempFilePaths;
						//上传图片
						uni.uploadFile({
							url : getApp().globalData.URL+'/sj/service/gzwz/imageUpload',
							...
							success: function (uploadFileRes) {
								...
								console.log('uni.uploadFile-imageUpload-success -> 返回：'+JSON.stringify(uploadFileResData));
								if('0' == uploadFileResData.resultCode){//上传图片成功
									let imgUrl = uploadFileResData.resultData.imgUrl;
									$this.shelveData.carouselImg = imgUrl;
								}
							}
						});
					},
