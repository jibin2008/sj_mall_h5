...
export default function MeScroll(options, isScrollBody) {
	...
	me.isScrollBody = isScrollBody || false; // 滚动区域是否为原生页面滚动; 默认为divDownScrolling = false; // 是否在执行下拉刷新的回调
...
MeScroll.prototype.extendDownScroll = function(optDown) {
	...
	MeScroll.extend(optDown, {
		...
		startTop: 100, // div时的scroll-top不一定为0, 此值用于控制最大的误差
...
MeScroll.prototype.touchmoveEvent = function(e) {
	...
	// divtouchmove,当触顶/底/左/右时,才会触发touchmove
	// divcrollTop不一定为0; 在iOS的APP中scrollTop可能为负数,不一定和startTop相等
	if (moveY > 0 && (
...
MeScroll.prototype.touchendEvent = function(e) {
	...
	} else if (!this.isScrollBody && this.getScrollTop() === this.startTop) { // div的滑动事件
...
MeScroll.prototype.endDownScroll = function() {
	...
	let endScroll = function() {
		...
		!me.isScrollBody && me.setScrollHeight(0) // div数据不满屏时仍可检查触发翻页
	}
	// 结束下拉刷新时的回调
	let delay = 0;
	...
	if (isBounce === false) {
		...
		window.bounceTouchmove = function(e) {
			...
			while (el !== document.body && el !== document) {
				...
				if (cls) {
					if (cls.contains('mescroll-touch')) { // 采用divescroll-uni,否则下拉仍然有回弹
