...
-->
<template>
	<div class="mescroll-empty" :class="{ 'empty-fixed': option.fixed }" :style="{ 'z-index': option.zIndex, top: option.top }">
		<image v-if="icon" class="empty-icon" :src="icon" mode="widthFix" />
		<div v-if="tip" class="empty-tip">{{ tip }}</div>
		<div v-if="option.btnText" class="empty-btn" @click="emptyClick">{{ option.btnText }}</div>
	</div>
</template>
