<template name="yq-avatar">
	<div>
		<div v-if="undefined == imgSrc.imgSrc || '' == imgSrc.imgSrc" class="logo_box tc" style="color: #8f8f94;background: 0 0;" @click="fSelect">
			<p class="f12">点击上传</p>
		</div>
		<image v-else :src="imgSrc.imgSrc" @click="fSelect" :style="[ imgStyle ]" class="my-avatar"></image>
		...
		<div class="oper-wrapper" :style="{display: styleDisplay}">
			<div class="oper">
				<div class="btn-wrapper" v-if="showOper">
					<div @click="fSelect"  hover-class="hover" :style="{width: btnWidth}"><text>重选</text></div>
					<div @click="fClose"  hover-class="hover" :style="{width: btnWidth}"><text>关闭</text></div>
					<div @click="fRotate"  hover-class="hover" :style="{width: btnWidth, display: btnDsp}"><text>旋转</text></div>
					<div @click="fPreview" hover-class="hover" :style="{width: btnWidth}"><text>预览</text></div>
					<div @click="fUpload"  hover-class="hover" :style="{width: btnWidth}"><text>上传</text></div>
				</div>
				<div class="clr-wrapper" v-else>
					...
					<div @click="fPrvUpload"  hover-class="hover" :style="{width: btnWidth}"><text>上传</text></div>
				</div>
			</div>
		</div>
	</div>
...
<style>
	.logo_box{ border-radius:1000px;border:1px solid #eee;width:52px; height:52px; line-height:48px;}
	.my-canvas{
		display: flex;
