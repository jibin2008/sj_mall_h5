<template>
	<div v-if="isShow" class="picker">
		<!-- 日期选择器 -->
		<div v-if="type!='time'" class="picker-modal">
			<div class="picker-modal-header">
				<div class="picker-icon picker-icon-zuozuo" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetYear('-1')"></div>
				<div class="picker-icon picker-icon-zuo" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetMonth('-1')"></div>
				<text class="picker-modal-header-title">{{title}}</text>
				<div class="picker-icon picker-icon-you" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetMonth('+1')"></div>
				<div class="picker-icon picker-icon-youyou" :hover-stay-time="100" hover-class="picker-icon-active" @click="onSetYear('+1')"></div>
			</div>
			<swiper class="picker-modal-body" :circular="true" :duration="200" :skip-hidden-item-layout="true" :current="calendarIndex" @change="onSwiperChange">
				<swiper-item class="picker-calendar" v-for="(calendar,calendarIndex2) in calendars" :key="calendarIndex2">
					...
					<div class="picker-calendar-view" v-for="(date,dateIndex) in calendar" :key="dateIndex" @click="onSelectDate(date)">
						<!-- 背景样式 -->
						<div v-show="date.bgStyle.type" :class="'picker-calendar-view-'+date.bgStyle.type" :style="{background: date.bgStyle.background}"></div>
						<!-- 正常和选中样式 -->
						<div class="picker-calendar-view-item" :style="{opacity: date.statusStyle.opacity, color: date.statusStyle.color, background: date.statusStyle.background}">
							<text>{{date.title}}</text>
						</div>
						<!-- 小圆点样式 -->
						<div class="picker-calendar-view-dot" :style="{opacity: date.dotStyle.opacity, background: date.dotStyle.background}"></div>
						<!-- 信息样式 -->
			...
			<div class="picker-modal-footer">
				<div class="picker-modal-footer-info">
					<block v-if="isMultiSelect">
						<div class="picker-display">
							...
							<div v-if="isContainTime" class="picker-display-link" :hover-stay-time="100" hover-class="picker-display-link-active"
							 :style="{color}" @click="onShowTimePicker('begin')">{{BeginTimeTitle}}</div>
						</div>
						<div class="picker-display">
							<text>{{endText}}日期</text>
							<text class="picker-display-text">{{EndTitle}}</text>
							<div v-if="isContainTime" class="picker-display-link" :hover-stay-time="100" hover-class="picker-display-link-active"
							 :style="{color}" @click="onShowTimePicker('end')">{{EndTimeTitle}}</div>
						</div>
					</block>
					<block v-else>
						<div class="picker-display">
				...
				<div class="picker-modal-footer-btn">
					<div class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancel">取消</div>
					<div class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirm">确定</div>
				</div>
			</div>
		</div>
		<!-- 时间选择器 -->
		<div v-if="showTimePicker" class="picker">
			<div class="picker-modal picker-time">
				<div class="picker-modal-header">
				...
				<picker-view class="picker-modal-time" indicator-class="picker-modal-time-item" :value="timeValue" @change="onTimeChange">
					<picker-view-column>
						<div v-for="(v,i) in 24" :key="i">{{i<10?'0'+i:i}}时</div>
					</picker-view-column>
					<picker-view-column>
						<div v-for="(v,i) in 60" :key="i">{{i<10?'0'+i:i}}分</div>
					</picker-view-column>
					<picker-view-column v-if="showSeconds">
						<div v-for="(v,i) in 60" :key="i">{{i<10?'0'+i:i}}秒</div>
					</picker-view-column>
				</picker-view>
				<div class="picker-modal-footer">
					<div class="picker-modal-footer-info">
					...
					<div class="picker-modal-footer-btn">
						<div class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancelTime">取消</div>
						<div class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirmTime">确定</div>
					</div>
				</div>
			</div>
		</div>
